...
try {
  for (const width of [1440, 390, 320]) {
    ...
    assert.equal(await todo.locator('[data-todo-event]').count(), 13)
    ...
    assert.equal(await todo.getByRole('progressbar').getAttribute('aria-valuenow'), '2')
  ...
  console.log('PASS: TODO cards, local dates, completion and property preservation, filters, expand, settings dismissal, pinning and drag, keyboard controls, modal reuse and light/dark desktop/mobile layouts')
