...
try {
  for (const width of [1440, 390, 320]) {
    ...
    assert.equal(await page.getByLabel('名称', { exact: true }).inputValue(), '全屏保留的草稿')
    ...
    await sidebar.getByRole('button', { name: '课程作业 / 实验', exact: true }).click()
    assert.equal(await page.getByLabel('名称', { exact: true }).inputValue(), '全屏保留的草稿', 'Switching tabs should retain assignment edits')
  ...
  console.log('PASS: peer sidebar tabs, daily task lights, empty cells and overdue tasks, 7/14-day ranges, year/midnight rollover, fullscreen draft retention, edit/save/completion, 320px/390px/desktop layouts and dismissal')
} catch (error) {
