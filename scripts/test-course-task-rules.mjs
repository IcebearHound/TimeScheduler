...
try {
  for (const width of [1440, 390, 320]) {
    ...
    await page.getByRole('button', { name: '电路原理 · 实验编号与跳过', exact: true }).click()
    ...
    await page.getByRole('button', { name: '电路原理 · 实验事件类型', exact: true }).click()
    await page.getByRole('dialog', { name: '整行事件类型' }).getByRole('button', { name: '作业', exact: true }).click()
    await page.getByText('该行事件类型已更新，可撤销', { exact: true }).waitFor()
    assert.ok((await stored()).events.every(([, e]) => e.properties.taskKind === '作业'))
    await page.getByRole('button', { name: '电路原理 · 作业事件类型', exact: true }).click()
    await page.getByRole('dialog', { name: '整行事件类型' }).getByRole('button', { name: '实验', exact: true }).click()
    await page.getByText('该行事件类型已更新，可撤销', { exact: true }).waitFor()
    assert.equal((await stored()).events.filter(([, e]) => e.properties.taskKind === '实验报告').length, 6)
    ...
    assert.equal(await page.locator('tbody [data-assignment-type]').count(), 2)
