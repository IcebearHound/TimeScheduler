...
const mockAI = createServer(async (req, res) => {
  ...
  res.end(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ intent: 'edit', message: '请确认修改备注', actions: [{ op: 'update_event', id: event.id, changes: { properties: { ...event.properties, notes: 'AI 更新的备注' } } }] }) } }] }))
})
...
try {
  ...
  await page.getByRole('button', { name: '＋ 添加作业 / 实验', exact: true }).click()
  await page.getByRole('menuitem', { name: '从表格获取', exact: true }).click()
  ...
  await page.getByRole('button', { name: '关闭从表格获取' }).click()
  ...
  await page.getByText('外部 MCP 连接', { exact: true }).click()
  ...
  await page.getByLabel('发送给 Agent').fill('给第三次作业添加备注')
  ...
  const encrypted = await page.evaluate(() => new Promise((resolve, reject) => { const request = indexedDB.open('time-scheduler-private'); request.onsuccess = () => { const db = request.result; const get = db.transaction('vault').objectStore('vault').get('ai-api-profiles'); get.onsuccess = () => { resolve(JSON.stringify(get.result)); db.close() }; get.onerror = reject }; request.onerror = reject }))
