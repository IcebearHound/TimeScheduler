...
try {
  for (const width of [390, 320, 1440]) {
    ...
    await page.route(/^https:\/\/(api\.deepseek\.com|api\.anthropic\.com|generativelanguage\.googleapis\.com)\//, async route => {
      ...
      const raw = JSON.stringify({ intent: 'edit', message: '请确认操作预览', actions })
    ...
    for (const preset of ['deepseek', 'anthropic', 'gemini']) {
      if (!await page.getByRole('dialog', { name: '设置', exact: true }).isVisible()) await page.getByRole('button', { name: 'AI 设置', exact: true }).click()
      ...
      await page.getByLabel('发送给 Agent').fill('添加手机 AI 作业')
      ...
      if (width < 500) await page.locator('[data-mobile-sheet="日程 Agent"]').waitFor({ state: 'hidden' })
      await openAI()
    ...
    const vault = await page.evaluate(() => new Promise((resolve, reject) => { const r = indexedDB.open('time-scheduler-private'); r.onsuccess = () => { const db = r.result; const tx = db.transaction('vault'); const a = tx.objectStore('vault').get('ai-api-profiles'), b = tx.objectStore('vault').get('device-key'); tx.oncomplete = () => { resolve({ encrypted: a.result.data instanceof ArrayBuffer, extractable: b.result.extractable, raw: JSON.stringify(a.result) }); db.close() }; tx.onerror = reject }; r.onerror = reject }))
    ...
    await page.waitForFunction(() => document.querySelectorAll('[aria-label="API 配置"] option').length === 3)
    await page.getByLabel('发送给 Agent').fill('再添加一项任务')
    for (const mode of ['401', 'network', 'invalid']) {
      responseMode = mode
      const before = await count()
      await page.getByRole('button', { name: '发送', exact: true }).click()
      await page.getByRole('button', { name: '发送', exact: true }).waitFor({ state: 'visible' })
      await page.waitForFunction(() => [...document.querySelectorAll('[role="alert"]')].some(el => !el.textContent.includes('正在')))
