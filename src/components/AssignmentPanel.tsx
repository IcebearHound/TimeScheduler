...
export default function AssignmentPanel() {
  ...
  const [numberReview, setNumberReview] = useState<{ rows: AssignmentRow[]; suggestions: TaskNumberSuggestion[]; snapshot: Snapshot; revision: string } | null>(null)
  ...
  const quickChange = (id: string, change: TaskQuickChange) => {
    const result = transaction.current.then(async () => {
      if ('completed' in change) { useEventStore.getState().setCourseTaskCompleted(id, change.completed); setMessage('快捷调整已保存，可撤销'); return }
      const current = captureArchive()
      const actions = 'number' in change ? setCourseTaskNumberActions(current, id, change.number) : 'kind' in change ? setCourseTaskKindActions(current, id, change.kind) : setCourseTaskSkipActions(current, id, change.skipped)
  ...
  const courseOptions = [...chains.values()].map(chain => {
  ...
  const prepare = async (book: XLSX.WorkBook) => {
    ...
    const actions = assignmentActions(snapshot, rows), revision = await snapshotRevision(snapshot), suggestions = detectTaskNumbers(rows)
    setPreview(null)
    if (suggestions.length) setNumberReview({ rows, suggestions, snapshot, revision })
    else setPreview({ rows, actions, revision })
  }
  return <div className="assignment-panel space-y-5">
    ...
    {showAllCourses && <div className="space-y-3" aria-label="课程任务线路">
      ...
      {courses.map(c => {
        ...
        return <div key={c.id} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
          <div className="flex items-center justify-between"><strong style={{ color: c.color }}>{c.name}</strong><button className="workspace-button" onClick={() => startNew(c.id)}>＋ 添加任务</button></div>
    ...
    {mode === 'quick' && <AppPanel title={editing ? '修改任务详情' : '快捷添加作业 / 实验'} onClose={() => setMode(null)}><form ref={editor} className="assignment-form" onSubmit={event => { event.preventDefault(); void run(async () => {
      ...
      if (editing) {
        ...
        if (creatingCourse) {
          if (!form.course.trim()) throw new Error('请输入新课程名称')
          if (snapshot.eventChains.some(c => c.name.trim() === form.course.trim())) throw new Error('已有同名课程，请在课程下拉框中选择已有课程')
      ...
      {editing ? <label>所属事件链<select aria-label="所属事件链" className="workspace-input" value={form.course} onChange={e => setForm({ ...form, course: e.target.value })}>{courseOptions.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</select></label> : <>
        <label>课程<select required aria-label="课程" className="workspace-input" value={creatingCourse ? '__new_course__' : form.course} onChange={e => { const isNew = e.target.value === '__new_course__'; setCreatingCourse(isNew); setForm({ ...form, course: isNew ? '' : e.target.value }); setMessage('') }}><option value="" disabled>选择已有课程</option>{courseOptions.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}<option value="__new_course__">＋ 新建课程</option></select></label>
      ...
      {form.kind === '实验课' && <div className="assignment-form-wide space-y-2 rounded-lg border p-3 dark:border-slate-700">
        ...
        {!hasAcceptance && <><label>新增验收截止时间<input type="datetime-local" className="workspace-input" value={acceptance} onChange={e => setAcceptance(e.target.value)} /></label><button type="button" className="workspace-button" onClick={() => {
          const target = editing || !creatingCourse ? chains.get(form.course) : undefined
    ...
    {mode === 'table' && <AppPanel title="从表格获取" onClose={() => setMode(null)}><section className="space-y-3">
      ...
      <p className="text-xs text-slate-500">支持 XLSX、XLS、CSV 或粘贴表格。表头：课程、名称、截止日期、类别、开始时间、编号、实验关联编号、提交链接、提交方式、内容、备注。编号列或任务名称中的疑似次数将弹窗确认后导入。类别为实验课、实验验收、实验报告、作业或考试；实验课和考试须填开始时间，截止日期填写结束时间。同一次实验可使用相同关联编号。同名事项需填写类别；仅更新链接时可省略时间。</p>
      ...
      {preview && <div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800"><h4>导入预览 · {preview.rows.length} 项</h4><div className="max-h-52 overflow-auto">{preview.rows.map((r, i) => <p key={i} className="py-1 text-xs break-all">{r.course} / {r.name} · {r.deadline || '保留截止时间'}{r.confirmedNumber !== undefined ? ` · 编号 ${r.confirmedNumber}` : ''} · {r.link || '无链接'}</p>)}</div><button disabled={busy} className="workspace-button primary" onClick={() => void run(async () => { await applyActions(preview.actions, preview.revision); setMessage(`已导入 ${preview.rows.length} 项，可整体撤销`); setPreview(null) })}>应用到事件链</button><button className="workspace-button" onClick={() => setPreview(null)}>取消</button></div>}
      {message && <p role="status" className="text-sm text-indigo-600">{message}</p>}
    </section></AppPanel>}
    {numberReview && <TaskNumberImportPanel rows={numberReview.rows} suggestions={numberReview.suggestions} onClose={() => setNumberReview(null)} onConfirm={async rows => { setPreview({ rows, actions: assignmentActions(numberReview.snapshot, rows), revision: numberReview.revision }); setNumberReview(null) }} />}
