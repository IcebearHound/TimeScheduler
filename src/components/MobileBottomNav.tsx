import { FolderKanban, ListTodo, Plus, Sparkles, Grid2X2 } from 'lucide-react'
...
export default function MobileBottomNav() {
  ...
  return <nav className="mobile-bottom-nav" aria-label="主导航">
    ...
    <button onClick={() => { resetPanels(); ui.openRightPanelTab('todo') }} className="mobile-nav-item"><ListTodo size={20} /><span>待办</span></button>
    ...
    <button aria-label="日程 Agent" onClick={() => { resetPanels(); useWorkspaceStore.getState().open('ai') }} className="mobile-nav-item"><Sparkles size={20} /><span>Agent</span></button>
