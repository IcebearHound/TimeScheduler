...
export default function MCPConnectionPanel() {
  ...
  return <section className="space-y-3 text-xs">
    ...
    {!connected ? <><input aria-label="本机配对码" className="workspace-input" value={code} autoComplete="off" placeholder="8 位配对码" onChange={e => setCode(e.target.value)} /><button className="workspace-button" disabled={busy || !code} onClick={async () => { setBusy(true); try { await connectLocal(code); setConnected(true); setCode(''); setMessage(connectionStatus()) } catch (error) { setMessage((error as Error).message) } finally { setBusy(false) } }}>连接此存档（允许 MCP 读写）</button></> : <button className="workspace-button" disabled={busy} onClick={async () => { setBusy(true); try { await disconnectLocal(); setConnected(false); setMessage(connectionStatus()) } catch (error) { setMessage((error as Error).message) } finally { setBusy(false) } }}>断开 MCP 连接</button>}
    <p>{message}</p>
  </section>
}
