    ...
    <header className="mobile-header md:hidden">
      <div className="flex h-14 items-center gap-2 px-3">
        ...
        <div ref={userMenuRef} className="relative">
          ...
          {showUserMenu && <div className="absolute right-0 top-full z-[100] mt-2 w-64 rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800"><div className="border-b px-4 py-3 dark:border-slate-700"><p className="truncate text-sm font-medium">{cloudLogin || '本地用户'}</p><p className="mt-1 text-xs text-slate-500">{cloudMessage}</p></div><AccountMenuEntries onSelect={() => setShowUserMenu(false)} /></div>}
        </div>
      </div>
