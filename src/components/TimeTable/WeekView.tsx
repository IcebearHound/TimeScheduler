...
export default function WeekView() {
  ...
  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = 6 * slotH }, [currentDate, slotH])
  ...
  const filteredEvents = useMemo(() => {
    ...
    return Array.from(events.values()).filter(e =>
      e.startTime < end && e.endTime > viewDays[0] && !filterTypeIds.has(e.typeId)
    )
  }, [events, viewDays, filterTypeIds])
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-slate-800 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={() => { const p = new Date(currentDate); p.setDate(p.getDate() - dayCount); setCurrentDate(p) }}
            className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg"><ChevronLeft className="w-5 h-5 text-slate-600 dark:text-slate-300" /></button>
          ...
          <button onClick={() => { const n = new Date(currentDate); n.setDate(n.getDate() + dayCount); setCurrentDate(n) }}
            className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg"><ChevronRight className="w-5 h-5 text-slate-600 dark:text-slate-300" /></button>
            ...
            {DAY_OPTIONS.map(n => (
              <button key={n} onClick={() => setDayCount(n)}
                className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                  dayCount === n ? 'bg-white dark:bg-slate-600 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                }`}>{n}天</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => setCurrentDate(new Date())}
            className="px-3 py-1.5 text-xs font-medium rounded-lg bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-900/40 shadow-sm transition-colors">
            今天
          </button>
          <button onClick={() => {
            setDayCount(7)
            ...
            className="px-3 py-1.5 text-xs font-medium rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-200 dark:hover:border-blue-700 shadow-sm transition-colors">
            📅 标准周
          </button>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto">
        {/* 星期头 — sticky 置顶 */}
        <div className={`grid gap-px bg-slate-200 dark:bg-slate-700 sticky top-0 z-[5] shadow-sm`} style={{ gridTemplateColumns: `repeat(${dayCount}, 1fr)` }}>
        {viewDays.map((d, i) => {
          const dow = d.getDay() || 7
          const isToday = d.toDateString() === new Date().toDateString()
          return (
            <div key={d.toISOString()} className={`bg-white dark:bg-slate-800 px-4 py-3 ${isToday ? 'ring-2 ring-blue-500 ring-inset' : ''}`}>
              <div className={`font-bold ${isToday ? 'text-blue-600 dark:text-blue-400' : 'text-slate-900 dark:text-white'}`}>{DAY_NAMES[dow - 1]}</div>
              <div className={`text-sm ${isToday ? 'text-blue-500 dark:text-blue-400 font-semibold' : 'text-slate-500 dark:text-slate-400'}`}>{d.toLocaleDateString('zh-CN', { month: 'numeric', day: 'numeric' })}</div>
            </div>
          )
        })}
      </div>
        ...
        {/* 每日列 */}
        <div className={`grid gap-px bg-slate-200 dark:bg-slate-700`} style={{ gridTemplateColumns: `repeat(${dayCount}, 1fr)` }}>
