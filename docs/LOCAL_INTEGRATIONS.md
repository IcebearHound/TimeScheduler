...
日期总览从今天开始，每门课程一行、每天一列，自动适应宽度，也支持未来 7／14／30 天。灯珠使用实验（烧瓶）、作业（线圈本）、考试（学位帽）图标，图标统一缩为 12px，灯珠为 20px，保留移动端 48px 点击区域；TODO 仍区分实验课、验收、报告、作业和考试五种事项。实验课和考试按开始日期显示，截止事项按截止日期；颜色表示待处理、今日截止、逾期或完成，悬浮提示和无障碍名称包含类别、名称、时间与状态。逾期未完成任务单独列出；“全部课程任务”保留按课程查看历史与远期任务的入口。
...
- 单击／点按切换完成状态和颜色；双击／双点打开详情；右键／长按快速调整完成状态、编号、当前任务类型或跳过。为区分双击，单击有约 500ms 延迟。键盘 Enter/Space 切换，Shift+F10 打开快捷菜单。移动端保留较大触控区域，支持横向滑动日期表格、底部详情面板和固定保存按钮。
- 课程名右下角的类型图标可切换作业／实验／考试，批量修改该课程行已有任务，保留时间、详情和完成状态，整批可撤销。切回实验时恢复记住的实验子类型；普通作业转为实验验收。同一事件链的不同事项始终处于同一行，日期相同时上下排列。
- 课程名右下角的滑杆图标打开编号与跳过面板；按类型选择任意基准事项及非负整数（含 0），自动生成之前和之后的编号，负数留空，不再检查前面的实验数量。作业、实验、考试分别计数，切换类型时保留各自设置，同一 labGroupId 的实验课、验收和报告共享编号。
...
面板灯珠单击切换完成状态，双击打开详情，右键或移动端长按打开快捷菜单；可修改当前任务类型、设置编号基准、跳过 / 恢复本次。作业只调整本项，同组实验课、验收和报告共同跳过 / 恢复。单次操作使用任务属性 `taskSkipOverride: "skip" | "keep"`，优先于课程日期规则，不会影响同日其他任务。课程名右下角的类型图标仍批量修改整行，滑杆图标打开分类型编号预览和跳过规则面板。
