...
点击顶部或手机底部的 **Agent** 打开对话。电脑默认吸附右边栏，拖动标题栏可移出为悬浮窗、自由移动，靠近左右边缘时显示吸附预览，松开即停靠。也可使用标题栏的停靠位置按钮选择左边栏、右边栏或自由悬浮。停靠会为对话留出侧栏空间，支持原有侧栏宽度调整；右侧 TODO／实验作业标签可切回原面板。停靠位置和悬浮坐标在本机保存，拖动及切换位置保留草稿、对话和 API 配置。电脑仍支持放大、最小化，手机使用适配可视区域与软键盘的对话窗。关闭窗口会保留草稿，进行中的请求仍会完成并记入历史。窗口支持 Escape 和浏览器返回关闭，嵌套的导入窗口优先关闭。

输入区采用圆角卡片，上方输入、下方操作：**＋** 展开上传文件和导入课程表，**模型名称** 展开模型/API 选择，右侧圆形箭头发送；生成中箭头切换为停止按钮。没有语音输入或“完全访问”入口。桌面 Enter 发送、Shift+Enter 换行，中文输入法确认候选字时不会发送；手机 Enter 保留换行。

点击模型名称下的 **管理 API 配置** 可添加、编辑、删除多个服务商配置，配置区内提供高级连接选项、网页读取密钥和外部 MCP 连接；通用设置不再提供 AI 配置。保存后点击“完成配置”即可继续聊天。密钥独立加密保存在此浏览器，不写入对话历史、日程备份或云同步。

顶部 **＋** 新建对话；当前没有消息且草稿为空时按钮禁用，重复调用也不会新增空记录。历史按钮打开按“今天／昨天／最近 7 天／更早”分组的卡片列表，每项显示标题、一行摘要、时间，当前对话与草稿有轻量标记。点击卡片继续对话，“更多”中重命名或删除；可搜索标题、消息或草稿内容，无匹配时显示空状态。消息、生成文件、未发送草稿和每段对话的 API／模型选择会保存在当前浏览器，刷新后恢复；切换对话不会携带其他对话的消息上下文。上传附件原文件和读取到的网页正文不持久化，恢复后需重新提供；未确认的操作预览在切换或刷新后需重新生成。存储失败会在窗口内提示，不会静默丢弃当前页面消息。历史记录独立于日程备份与云同步。
...
在 Agent 输入框下方打开 **模型名称 → 管理 API 配置 → 外部 MCP 连接**，输入终端的配对码。网页保持打开即可读写此浏览器存档；不需要解锁旧版服务端 AI 凭据库。GitHub Pages 来源需由部署者将 `TIMESCHEDULER_ALLOWED_ORIGIN` 设置为该网站来源。
...
日期总览从今天开始，每门课程按事件类型分行、每天一列（按 chainId + typeId 分组，一门课程可有多行）。行首保留课程名，右侧用图标表示并切换该行事件类型，自动适应宽度，也支持未来 7／14／30 天。灯珠使用实验（烧瓶）、作业（线圈本）、考试（学位帽）图标，图标统一缩为 12px，灯珠为 20px，保留移动端 48px 点击区域；TODO 仍区分实验课、验收、报告、作业和考试五种事项。实验课和考试按开始日期显示，截止事项按截止日期；颜色表示待处理、今日截止、逾期或完成，悬浮提示和无障碍名称包含类别、所属事件链、名称、时间与状态。逾期未完成任务单独列出；“全部课程任务”保留按课程查看历史与远期任务的入口。

TODO 按“置顶 → 重点事项 → 近期安排 → 课程任务”排列。顶部将事件与课程任务的待完成数、已完成数和总数分开统计；课程任务仅统计今天及随后六天（本地日期），实验课和考试按开始时间，验收、报告和作业按截止时间计入。“课程任务 · 按时间排序”仍统一列出所有课程的五类事项，不受置顶、重点、手动拖拽、近期天数及其他安排的来源开关影响，历史逾期和远期事项不会遗漏。支持全部／待完成／已完成筛选。实验课与考试默认在结束后自动完成，也可手动切换；手动状态持续保留。验收、报告、作业各自勾选，互不影响。已跳过的任务不进入 TODO 和统计，也不触发浏览器通知。

- 快捷添加通过下拉框选择已有课程，按事件链 ID 精确保存；同名课程以类型和序号区分，无需重命名。只有明确选择“新建课程”才会创建事件链，重复名称会提示选择已有课程。表格导入仍按课程名匹配，同名链需先明确归属。
- 双击／双点灯珠打开详情，修改名称、上课时间或截止时间、提交方式、提交链接、内容、备注及完成状态；右键／长按打开简要快捷菜单。
...
- 单击／点按切换完成状态和颜色；双击／双点打开详情；右键／长按快速调整完成状态、编号、当前任务类型或跳过。单击立即显示完成状态和颜色；为区分双击，仅将持久化延迟约 500ms，双击会取消这次单击并打开详情。保存失败会恢复原状态，单击后离开面板仍会保存。单次状态修改使用轻量本地事务，保留撤销，避免重复校验及重建整个存档。键盘 Enter/Space 切换，Shift+F10 打开快捷菜单。移动端保留较大触控区域，支持横向滑动日期表格、底部详情面板和固定保存按钮。
- 课程名右侧的类型图标可选择任意已有事件类型，仅批量修改当前课程、当前类型这一行的任务，保留时间、详情和完成状态，整批可撤销。切回实验时恢复记住的实验子类型；普通作业转为实验验收。修改后任务归入对应事件类型行；同类型任务日期相同时按时间上下排列。
- 点击课程名打开编号与跳过面板；编号和日期规则按事件链独立保存，避免不同课程互相影响；按类型选择任意基准事项及非负整数（含 0），自动生成之前和之后的编号，负数留空，不再检查前面的实验数量。作业、实验、考试分别计数，切换类型时保留各自设置，同一 labGroupId 的实验课、验收和报告共享编号。
...
- 快捷添加可填写“本次编号”，支持 0–100000；留空沿用现有编号规则，填写后以首个新任务为基准前后递推，同组实验共号。新任务和编号规则在同一事务保存、可整体撤销。可每 1–12 周重复，共 1–52 次（含首项），每次实验组合使用独立关联编号，一次最多 200 个事件；规则按浏览器／本机 MCP 服务所在时区维持钟面时间。生成后可逐项编辑。作业和实验始终使用对应默认类型，不会错误地保存为课程类型。
...
表格的“编号／序号／次数”列，以及名称中的“第三次”“作业四”“实验（2）”可自动识别为编号建议；不会从普通日期中取号。识别后弹窗逐项确认，可修改、取消勾选或选择“不导入编号”，确认后仍需应用预览才写入。编号列优先于名称；同课程同类别多个确认编号必须与时间顺序和跳过规则一致，否则提示调整，不部分写入。
...
| `set_course_task_row_type` | `{revision,courseId,sourceTypeId,targetTypeId}` | 精确修改面板当前课程、当前类型这一行的事件类型，不影响其他课程及该课程的其他行 |
